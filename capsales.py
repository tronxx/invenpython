...
class Capsales:
  """Esta es una aplicación Captura de Salidas Especiales"""
       
  def __init__(self):
       
    #Establecemos el archivo Glade
    self.gladefile = dirprogs_z + "capsales.glade"
...
# execute SQL statement
  ...
  def asigna_tipent(self, tipo_z):
    ...
    if self.tipent_z == "S":
       self.objetos_invisibles=["edt_nomptovt", "edt_ptovta", "lbl_ptovta", \
         "lbl_mayoris", "edt_mayoris", "edt_nommay", "chk_entsn", "lbl_numfac", "edt_numfac"]
       self.objetos_visibles=["lbl_preciovta", "edt_preciovta"]
       self.tipospago_z = ["EFECTIVO", "A.C.L", "CAJA AHORRO"]
    elif self.tipent_z == "M":
         self.objetos_visibles = ["lbl_mayoris", "edt_mayoris", "edt_nommay", "chk_entsn",\
          "lbl_numfac", "edt_numfac"]
         self.objetos_invisibles=["edt_nomcli", "lbl_cliente", "chk_cancel", \
         "lbl_poblac", "edt_poblac", "lbl_tipago", "edt_tipago", "lbl_vend", \
         "edt_vend", "edt_nomvnd", "chk_sino", "lbl_ptovta", "edt_ptovta", \
         "edt_nomptovt", "lbl_preciovta", "edt_preciovta"]
    elif self.tipent_z in sabanas_venta: 
       self.objetos_invisibles=["lbl_preciovta", "edt_preciovta", \
            "lbl_mayoris", "edt_mayoris", "edt_nommay", "chk_entsn", \
            "lbl_numfac", "edt_numfac"]
       self.objetos_visibles=["edt_nomptovt", "edt_ptovta", "lbl_ptovta"]
       if self.tipent_z == "V":
          self.tipospago_z = ["CREDITO", "CONTADO"]
       elif self.tipent_z in sabanas_sin_tipo_pago:
           self.objetos_invisibles.append("lbl_tipago")
           self.objetos_invisibles.append("edt_tipago")
    for objeto_z in self.objetos_invisibles:
        #print objeto_z, " 1.-Invisible", visibles_z
        self.wTree.get_widget(objeto_z).set_child_visible(False)
    for objeto_z in self.objetos_visibles:
        #print objeto_z, " 2.-Invisible", visibles_z
        self.wTree.get_widget(objeto_z).set_child_visible(True)
  ...
  def on_edt_codigo_focus_out_event(self, widget, tipo=None):
      ...
      if ( self.busca_inv(codigo_z) == True):
         ...
         self.wTree.get_widget("edt_preciovta").set_text(str(inven['costos']))
      businven_z = False
  ...
  def on_btn_nuevo_clicked(self, widget):
      global modo_z
      modo_z = NUEVO
      dlg_datosentrada = self.wTree.get_widget("dlg_datosentrada")
      dlg_datosentrada.run()
      edt_almacen  = dlg_datosentrada.get_widget("edt_almacen")
      edt_numero  = dlg_datosentrada.get_widget("edt_numero")
      edt_fecha  = dlg_datosentrada.get_widget("edt_fecha")
  ...
  def busca_pob(self, pob_z = ''):
      ...
      if record <> None:
        ...
        cursor.execute(sql_z)
        result_z = cursor.fetchall()
        datosbuscados_z = utils.busca_datos(result_z, "Poblacion:Num", "Seleccione la Poblacion")
        miresp_z = datosbuscados_z.split(":")
        resp_z = utils.StrToInt(miresp_z[-1])
        if resp_z == gtk.RESPONSE_OK:
           pob_z   = miresp_z[0]
  ...
  def busca_mayoris(self, mayoris_z = ''):
      ...
      sql_z = "select codigo, nombre from mayoris where codigo = '" + mayoris_z + "'"
      sql_z = sql_z + " and cia = " + utils.IntToStr(cia_z)
      cursor = mydb.cursor()
      cursor.execute(sql_z)
      record = cursor.fetchone()
      if record <> None:
        ...
        edt_vend.set_text(mayoris['codigo'])
        edt_nombre.set_text(mayoris['nombre'])
        resp_z = True
      else:
        ...
        resp_z = utils.StrToInt(miresp_z[-1])
        if resp_z == gtk.RESPONSE_OK:
           mayoris['codigo']    = miresp_z[0]
           mayoris['nombre']    = miresp_z[1]
           edt_vend.set_text(mayoris['codigo'])
           edt_nombre.set_text(mayoris['nombre'])
           resp_z = True
        else:
  ...
  def agrega_nuevo_ren(self):
      ...
      if self.tipent_z == "V":
         ...
         #utils.msgdlg("idTipo de pago :" + repr(idtipago_z))
         #Fin de If
      #Fin de If
      if len(tipago_z) > 1:
         tipago_z = tipago_z[0]
      ...
      if self.tipent_z == "M":
         if self.wTree.get_widget("chk_entsn").get_active() == True:
            siono_z = "S"
         else:
            siono_z = "N"
         #End If
  ...
  def nueva_modif_entradas(self):
      ...
      if self.tipent_z == "M":
         ...
         if self.wTree.get_widget("chk_entsn").get_active() == True:
           ...
           modent_z = "N"
         #End If
         nommay_z = def_tablas.busca_nombre(mydb, mayoris_z, cia_z, MAYORIS)
         ncli_z   = def_tablas.busca_iddato(mydb, nommay_z, def_tablas.CONCEPTOS)
         facmay_z = utils.StrToInt(self.wTree.get_widget("edt_numfac").get_text())
         if ( self.busca_mayoris(mayoris_z) <> True):
      ...
      entradas['facpro']  = ''
      entradas['prove']  = mayoris_z
      entradas['perenvrec']  = ncli_z
      entradas['planp']  = facmay_z
      entradas['status']  = 'A'
      entradas['coniva']  = modent_z
  ...
  def encab(self):
      ...
      self.arch_z.write(self.condensado_on + "capsales " + self.condensado_off + \
        (hora_z + " " + cias['dir']).center(70) + "\n")
      self.arch_z.write(("Impresion de " + \
        def_tablas.tipoentra(self.tipent_z)[2] + " Pag:" + str(self.pag_z)).center(80) + "\n")
      ...
      if self.tipent_z == "M":
         mayoris_z = entradas['prove']
         nommay_z = def_tablas.busca_nombre(mydb, mayoris_z, cia_z, MAYORIS)
         self.arch_z.write("Mayorista: " + mayoris_z + " " + nommay_z + "\n")
      #End if
      ...
      self.arch_z.write("Poblacion ".ljust(10)+"|")
...
# execute SQL statement
  ...
  def busca_entrada(self, alm_z = '', numero_z = 0):
      sql_z = "select tipo, alm, numero, fecha, planp, prove, coniva, perenvrec "
      sql_z = sql_z + "from entradas where tipo = '" + self.tipent_z + "' and alm = '" 
      ...
      if record <> None:
         ...
         entradas['planp']     = record[4]
         entradas['prove']     = record[5]
         entradas['coniva']    = record[6]
         entradas['perenvrec'] = record[7]
         resp_z = True
      else:
         resp_z = False
      return ( resp_z)
  ...
  def despliega_datos(self):
      ...
      self.wTree.get_widget("edt_mayoris").set_text(entradas['prove'])
      nommay_z = def_tablas.busca_nombre(mydb, entradas['prove'], cia_z, MAYORIS)
      self.wTree.get_widget("edt_nommay").set_text(nommay_z)
      self.wTree.get_widget("edt_numfac").set_text(utils.IntToStr(entradas['planp']))
      self.wTree.get_widget("chk_entsn").set_active(entradas['coniva'] == 'S')
      self.lista_renglones(alm_z, numero_z)
  ...
  def lista_renglones(self, alm_z='', numero_z=0):
      ...
      for record in result:
          ...
          self.lst_renentra.append([ renentra['codinv'], descri_z, renentra['folsal'], renentra['serie'], costou_z, renentra['siono'], renentra['vend'], renentra['status'], renentra['conse'] ])
  ...
  def despliega_renglon(self, alm_z='', numero_z=0, conse_z=0):
      ...
      sql_z = sql_z + " and conse = " + utils.IntToStr(conse_z) 
      sql_z = sql_z + " and a.cia = " + utils.IntToStr(cia_z)
      codigo_z = ''
      vend_z = ''
      tipago_z = ''
      prvta_z = ''
  ...
  def limpia_campos(self):
      campos_z = [ "edt_codigo", "edt_descri", "edt_nomcli", "edt_vend", "edt_nomvnd", \
        "edt_poblac", "edt_tipago", "edt_preciovta", "edt_tipago", "edt_numero", \
        "edt_fecha", "edt_mayoris", "edt_nommay", "edt_numfac" ]
...
if __name__ == "__main__":
   hwg = Capsales()
   hwg.wTree.get_widget("win_capsales").connect("destroy", gtk.main_quit )
   gtk.main()
