...
# --Fin de generar cat�logo
  ...
  def genera_archivo_linea(self, linea_z):
      ...
      sql_z = "select linea, b.codigo, b.descri, a.codigo, a.descri, precio, a.empaqe, count(*) "
      ...
      #Fin If
      sql_z = sql_z + " group by linea, b.codigo, b.descri, a.codigo, a.descri, precio, a.empaqe"
      print sql_z;
      cursor = mydb.cursor()
      ...
      for record in result:
          ...
          if siimprimir_z == "SI" :
            ...
            if os.path.isfile(archimg_z) == False:
               ...
               artsinfoto_z.append([grupo_z, codigo_z, descri_z, precio_z])
            else:
               artconfoto_z.append([grupo_z, codigo_z, descri_z, precio_z])
            #End if
          #End if
      #Fin de For
      ...
      for miart_z in artconfoto_z:
          ...
          farchlin_z.write(descri_z + "<br>" + tumb_z + "</A>\n");
...
# --- Define archivo html x codigo -----
  def genera_arch_cod_html(self, codigo_z, sigcod_z, antcod_z, descri_z, pathlin_z, linea_z, siglin_z, precio_z):
          ...
          cursor.execute(sql_z)
          result = cursor.fetchall()
          ##--> Voy a buscar las Series cuando sea articulo con Serie
              ...
              if record[1] <> 0:
                 ...
                 for micolor_z in miscolores_z:
                    if micolor_z[0] == record [0]:
                       farchcod_z.write(primero_z + micolor_z[1] + ":")
                       farchcod_z.write(repr(int(micolor_z[2])))
                       primero_z = " "
...
# --- Define Buscar Colores -----
  def  busca_colores(self, codigo_z):
     global cia_z
     sibuscar_z = "NO"
     sql_z = "select tipo from inven where codigo = '" + codigo_z + "'"
     sql_z = sql_z + " and cia = " + repr(cia_z)
     cursor = mydb.cursor()
     cursor.execute(sql_z)
     ...
     if result[0] <> "ALF":
        ...
        if sibuscar_z <> "SI":
           sql_z = "select almac, substring(serie, length(serie) - 1, 2) as colores,"
           sql_z = sql_z + " count(*) from movart "
           sql_z = sql_z + "where codigo = '" + codigo_z + "' and salio <> 'S'"
           sql_z = sql_z + " and serie like '--%' and cia =" + repr(cia_z)
           sql_z = sql_z + " group by almac, colores";
        else:
           sql_z = "select almac, substring(serie, length(serie) - 1, 2) as colores,"
           sql_z = sql_z + " count(*) from movart "
           sql_z = sql_z + "where codigo = '" + codigo_z + "' and salio <> 'S'"
           sql_z = sql_z + " and length(serie) > 2 and cia =" + repr(cia_z)
           sql_z = sql_z + " group by almac, colores";
